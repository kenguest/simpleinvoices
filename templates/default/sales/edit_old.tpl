		...
		<tr>
			...
			{else}
				<td>
					...
					<option value="{$data.company.id}" selected="selected">{$data.company.name}</option>
				{foreach from=$data.companys item=vendor}
...
{* $default_invoice_number *}
	...
	<table class="invoice_header_right">
		...
		<tr>
			...
			<td><input type="text" class="date-picker" name="select_date" id="date1" value='{$data.invoice.date}'></input></td>
		</tr>
	</table>
...
<!-- <div class="invoice_standard"> -->
	...
	{section name=line start=0 loop=70 step=1}
		{assign var=line value=$smarty.section.line.index}
		{if isset($invoice_lines.$line.reference)}
		<tr id="line_{$smarty.section.line.index}" num="{$smarty.section.line.index}">
			...
			{else}
				<select id="select_products{$smarty.section.line.index}" name="select_products{$smarty.section.line.index}" class="line_reference" num="{$smarty.section.line.index}" tabindex="{$smarty.section.line.index+1}">
					...
					<option value="{$invoice_lines.$line.id}" selected="selected">{$invoice_lines.$line.reference}</option>
					{foreach from=$data.products item=product}
						<option value="{$data.products.id}">{$product.reference}</option>
					{/foreach}
				</select>
			{/if}
			...
			<td class="qty">
				<input type=text id="i_quantity{$smarty.section.line.index}" name="i_quantity{$smarty.section.line.index}" num="{$smarty.section.line.index}" size="2" value="{$invoice_lines.$line.quantity}" tabindex="{$smarty.section.line.index+1}" />
			</td>

			<td class="unit_price">
				<input type=text name="i_unit_cost{$smarty.section.line.index}" id="i_unit_cost{$smarty.section.line.index}" size="5" num="{$smarty.section.line.index}" value="{$invoice_lines.$line.unit_cost}" tabindex="{$smarty.section.line.index+1}" />
			</td>
...
<tr>
        <td colspan=2><textarea input type=text name="invoice_note" rows=5 cols=70 WRAP=nowrap>{$invoice.note}</textarea></td>
</tr>
